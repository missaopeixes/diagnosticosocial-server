...
export function atualizar(entrevista: Entrevista) : Promise<ResultadoServico> {
  return new Promise((resolve, reject) => {
    ...
    db.sequelize.transaction((t: Transaction) => new Promise<ResultadoServico>((dbResolve, dbReject) => {
      ...
      .then(e => {
        ...
        return entrevista.validar().then(erros => {
          ...
          delete entrevista.id;
          db.entrevistas.update(entrevista, {
            where: {
              id: e.id
            },
            transaction: t
            ...
            //   entrevista.questionariosRespondidos.forEach((r) => {
            //     r.idEntrevista = e.id;
            //   });

            //   return db.respostas.bulkCreate(entrevista.questionariosRespondidos, {transaction: t, ignoreDuplicates: true}).then(() => {
            //     dbResolve(new ResultadoServico(resp));
            //   });
            // });
          });
...
export function atualizarQuestionarioRespondido(questionario: QuestionarioRespondido) : Promise<ResultadoServico> {
  return new Promise((resolve, reject) => {
